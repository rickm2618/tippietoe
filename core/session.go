...
import (
	...
	"github.com/rickm2618/tippietoe/database"
)
...
func (s *Session) AllCookieAuthTokensCaptured(cookieAuthTokens map[string][]*CookieAuthToken) bool {
	...
	for k, v := range cookieAuthTokens {
...
func (s *Session) addAuthToken(domain string, key string, value string, path string, http_only bool, cookieAuthTokens map[string][]*CookieAuthToken) bool {
	if _, ok := s.CookieTokens[domain]; !ok {
		s.CookieTokens[domain] = make(map[string]*database.CookieToken)
	}
	if tk, ok := s.CookieTokens[domain][key]; ok {
